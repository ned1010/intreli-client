...
export function useDocumentAutocomplete(
    ...
    const detectTrigger = () => {
      ...
      if (triggerMatch) {
        ...
        console.log('Document filtering:', {
          ...
          query,
          filteredDocs: filtered.map(d => d.name)
        });
        ...
        setIsOpen(prev => {
          ...
          console.log('Setting isOpen:', { shouldBeOpen, filteredCount: filtered.length, prev });
  ...
  }, [inputValue, cursorPosition, documents, maxResults, debounceMs]);
